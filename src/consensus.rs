...
use crate::economics::{EconomicsError, RewardCalculator, TokenBalance};
...
impl ConsensusState {
    ...
    pub fn increment_height(&mut self) {
        ...
        tracing::info!(
            "🔗 CONSENSUS: Block height incremented to {}",
            self.current_height
        );
    }
    ...
    pub fn mark_rewarded(&mut self, height: u64) {
        ...
        tracing::warn!(
            "🔒 REWARD: Height {} marked as rewarded (prevents double-reward)",
            height
        );
    }
    ...
    pub fn validate_and_mark_block_reward(
        &mut self,
        block_id: BlockId,
    ...
    pub fn rollback_block_reward(&mut self, block_id: &BlockId) {
        if self.rewarded_blocks.remove(block_id) {
            tracing::warn!(
                "⚠️ FORK: Rolling back reward for block {}",
                hex::encode(block_id)
            );
        }
    }
    ...
    pub fn compute_subgraph_score(
        ...
        self.compute_subgraph_score_recursive(
            tip,
            dag,
            reputations,
            &mut visited,
    ...
    pub fn select_canonical_subgraph(
        &self,
        dag: &DAG,
        reputations: &HashMap<Address, f64>,
        ...
        tracing::info!(
            "🏆 Canonical subgraph selected: tip {} with score {}",
            hex::encode(&best_tip.unwrap_or([0u8; 32])[..8]),
            best_score
        );
    ...
    pub fn compute_finality_probability(
        &self,
        tx_id: TransactionId,
        dag: &DAG,
        volatility: f64,
    ...
    fn count_descendants_recursive(
        &self,
        tx_id: TransactionId,
        dag: &DAG,
        visited: &mut HashSet<TransactionId>,
...
impl VQVConsensus {
    ...
    pub fn select_quorum(&self, tx_id: TransactionId, tx_amount: u64) -> Vec<ValidatorId> {
        ...
        validator_ids.into_iter().take(quorum_size).collect()
    }
    ...
    pub fn select_quorums_batch(
        &self,
        transactions: &[(TransactionId, u64)],
    ) -> Vec<Vec<ValidatorId>> {
        transactions
            .par_iter()
    ...
    pub fn simulate_vote(&self, tx_id: TransactionId, tx_amount: u64, approve: bool) -> Vote {
        ...
        if let Some(validator_id) = quorum.first() {
            if let Some(validator) = self.validators.get(validator_id) {
                return Vote::new(*validator_id, tx_id, approve, validator.voting_weight());
            }
        }
    ...
    pub fn distribute_mining_reward(
        &mut self,
        validator_id: ValidatorId,
        reward: u64,
