...
#[derive(Parser)]
#[command(name = "aether", version = "1.3.3", about = "AETHER SEDC")]
struct Cli {
    #[command(subcommand)]
...
enum Tab { Overview, Send, Receive }
...
fn card(ui: &mut egui::Ui, add_contents: impl FnOnce(&mut egui::Ui)) {
    Frame::none().fill(CARD_BG).rounding(Rounding::same(8.0)).inner_margin(Margin::symmetric(12.0, 8.0)).show(ui, add_contents);
}
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::bottom("footer").min_height(28.0).show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if state.wallet { ui.label(egui::RichText::new("Wallet: active").size(11.0).color(Color32::GREEN)); }
            });
        });
        ...
        egui::SidePanel::left("tabs").resizable(false).exact_width(120.0).show(ctx, |ui| {
            ...
            for (tab, name) in tabs.iter().zip(names.iter()) {
                ...
                if ui.add(btn).clicked() { self.active_tab = tab.clone(); }
                ui.add_space(4.0);
            }
        });
...
impl AetherGui {
    fn show_overview(&mut self, ui: &mut egui::Ui, state: &GuiState, connected: bool) {
        ui.heading("Overview");
        ui.add_space(8.0);
        card(ui, |ui| { ui.label(egui::RichText::new("Balance").color(TEXT_SECONDARY)); ui.heading(egui::RichText::new(&state.balance).size(28.0).color(ACCENT)); });
        ui.add_space(8.0);
        card(ui, |ui| { ui.label(egui::RichText::new("Network").color(TEXT_SECONDARY)); ui.horizontal(|ui| { ui.label(format!("Peers: {}", state.peers)); ui.add_space(16.0); ui.label(format!("Transactions: {}", state.transactions)); }); });
        if !state.address.is_empty() {
            ui.add_space(8.0);
            card(ui, |ui| { ui.label(egui::RichText::new("Address").color(TEXT_SECONDARY)); ui.monospace(&state.address); });
        }
        if state.wallet && connected {
            ui.add_space(8.0);
            if ui.button(egui::RichText::new("Request Faucet").size(14.0)).clicked() {
                let addr = state.address.clone(); let url = self.rpc_url.clone(); let state = self.state.clone();
                std::thread::spawn(move || match rpc_call(&url, "aether_faucet", serde_json::json!([addr])) { Ok(r) => { let mut s = state.lock().unwrap(); s.send_result = format!("Faucet: {}", r); } Err(e) => { let mut s = state.lock().unwrap(); s.send_result = format!("Error: {}", e); } });
            }
        }
        if !state.send_result.is_empty() { ui.add_space(8.0); ui.label(&state.send_result); }
        if !state.dag_stats.is_empty() { ui.add_space(8.0); ui.separator(); ui.label(egui::RichText::new("Node Info").color(TEXT_SECONDARY).size(11.0)); ui.monospace(&state.dag_stats); }
    }

    fn show_send(&mut self, ui: &mut egui::Ui, state: &GuiState, connected: bool) {
        ui.heading("Send");
        if !state.wallet { card(ui, |ui| { ui.label("Create a wallet first in the Receive tab."); }); return; }
        ui.add_space(8.0);
        card(ui, |ui| { ui.label(egui::RichText::new("Recipient Address").color(TEXT_SECONDARY)); ui.text_edit_singleline(&mut self.recipient); ui.add_space(8.0); ui.label(egui::RichText::new("Amount (AETH)").color(TEXT_SECONDARY)); ui.text_edit_singleline(&mut self.amount); });
        ui.add_space(8.0);
        if ui.button(egui::RichText::new("Send Transaction").size(14.0)).clicked() && connected {
            let addr = self.recipient.clone(); let amt: u64 = self.amount.parse().unwrap_or(0); let url = self.rpc_url.clone(); let state = self.state.clone();
            std::thread::spawn(move || { if addr.len() == 64 && amt > 0 { match rpc_call(&url, "aether_sendTransaction", serde_json::json!([addr, amt])) { Ok(r) => { let mut s = state.lock().unwrap(); s.send_result = format!("Sent: {}", r); } Err(e) => { let mut s = state.lock().unwrap(); s.send_result = format!("Error: {}", e); } } } else { let mut s = state.lock().unwrap(); s.send_result = "Invalid address or amount".to_string(); } });
        }
        if !state.send_result.is_empty() { ui.add_space(8.0); ui.label(&state.send_result); }
    }
        ...
        if !state.wallet {
            card(ui, |ui| {
                ...
                if ui.button("Create Wallet").clicked() {
                    ...
                    s.address = w.address_string(); s.wallet = true; s.secret_key = w.secret_key_hex.clone(); s.mnemonic = w.mnemonic.unwrap_or_default(); s.status = "Wallet created".to_string(); s.status_color = Color32::GREEN;
                }
            });
            return;
        }
        card(ui, |ui| { ui.label(egui::RichText::new("Your Address").color(TEXT_SECONDARY)); ui.add_space(4.0); ui.monospace(&state.address); });
        if !state.mnemonic.is_empty() { ui.add_space(8.0); card(ui, |ui| { ui.colored_label(Color32::YELLOW, "Recovery Phrase (SAVE THIS)"); ui.add_space(4.0); ui.monospace(&state.mnemonic); }); }
    }

    fn refresh_bg(url: &str, state: Arc<Mutex<GuiState>>) {
        match rpc_call(url, "aether_getMiningStatus", serde_json::json!([])) {
            Ok(_) => { let mut s = state.lock().unwrap(); s.status = "Connected".to_string(); s.status_color = Color32::GREEN; }
            Err(_) => { let mut s = state.lock().unwrap(); s.status = "Disconnected".to_string(); s.status_color = Color32::RED; return; }
        }
        if let Ok(stats) = rpc_call(url, "aether_getDagStats", serde_json::json!([])) {
            let mut s = state.lock().unwrap();
            if let Some(tx) = stats.get("transaction_count").and_then(|v| v.as_u64()) { s.transactions = tx.to_string(); }
            if let Some(pc) = stats.get("peer_count").and_then(|v| v.as_u64()) { s.peers = pc.to_string(); }
...
fn load_icon() -> IconData {
    let w = 32; let h = 32; let mut rgba = vec![0u8; w * h * 4];
    for y in 0..h { for x in 0..w {
        let i = (y * w + x) * 4; let cx = x as f64; let cy = y as f64;
        let left = 16.0 - (cy - 3.0) * 0.4; let right = 16.0 + (cy - 3.0) * 0.4;
        ...
        if (in_a || crossbar) && !inner { rgba[i] = 0; rgba[i+1] = 180; rgba[i+2] = 255; rgba[i+3] = 255; }
        else { rgba[i] = 18; rgba[i+1] = 18; rgba[i+2] = 28; rgba[i+3] = 255; }
    }}
    IconData { rgba, width: w as u32, height: h as u32 }
}

async fn send_transaction_client(wallet_path: &str, receiver_hex: &str, amount: u64, fee: u64, rpc_url: &str, password: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
    let wallet = Wallet::from_file(wallet_path, password).await.map_err(|e| {
        if e.to_string().contains("Password required") { eprintln!("Password required. Use --password"); }
        e
    })?;
    let sender_address = wallet.address();
    let receiver_address: [u8; 32] = hex::decode(receiver_hex)?.try_into().map_err(|_| "Invalid receiver hex")?;
    ...
    let account_nonce = nonce_json["result"]["next_nonce"].as_u64().ok_or("No nonce")?;
    let tips_resp = client.post(rpc_url).json(&serde_json::json!({"jsonrpc":"2.0","method":"aether_getTips","params":[],"id":2})).send().await?;
    ...
    if let Some(tips) = tips_json["result"]["tips"].as_array() {
        for (i, tip) in tips.iter().take(2).enumerate() {
            if let Some(ts) = tip.as_str() { if let Ok(tb) = hex::decode(ts) { if tb.len() == 32 { parents[i].copy_from_slice(&tb); } } }
        }
    }
    let tx = Transaction::new(parents, sender_address, receiver_address, amount, fee, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH)?.as_millis() as u64, 0, account_nonce, vec![0u8; 64], wallet.public_key_bytes());
    let difficulty = Transaction::default_difficulty();
    println!("Mining transaction...");
    let start = std::time::Instant::now();
    let nonce = tx.mine_nonce(difficulty);
    println!("Mined nonce: {} ({:.2}s)", nonce, start.elapsed().as_secs_f64());
    ...
    if send_resp.status().is_success() { println!("Transaction sent! Amount: {} to {}", amount, receiver_hex); }
    else { eprintln!("Failed: {}", send_resp.text().await?); std::process::exit(1); }
    Ok(())
}
...
async fn balance_client(address_hex: &str, rpc_url: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    } else { eprintln!("RPC error"); std::process::exit(1); }
    Ok(())
}
...
fn run_gui(cfg: NodeConfig) -> eframe::Result<()> {
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async { if let Err(e) = aether_unified::node::run_node(cfg).await { tracing::error!("Node error: {}", e); } });
    });
    ...
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_inner_size(Vec2::new(640.0, 480.0)).with_min_inner_size(Vec2::new(480.0, 360.0))
            .with_icon(Arc::new(load_icon())).with_title("AETHER SEDC"),
        ..Default::default()
    };

    eframe::run_native("AETHER SEDC", options, Box::new(|_cc| Box::new(AetherGui {
        rpc_url, state: Arc::new(Mutex::new(GuiState::default())),
        recipient: String::new(), amount: String::new(), last_refresh: Instant::now(), active_tab: Tab::Overview,
    })))
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match &cli.command {
        ...
        Some(Commands::Wallet { action }) => match action {
            WalletAction::Create { path } => {
                ...
                let mut pw = String::new(); std::io::stdin().read_line(&mut pw)?;
                let pw = pw.trim();
                if pw.is_empty() { eprintln!("Password required"); std::process::exit(1); }
                wallet.to_file(path, Some(pw)).await?;
                println!("Wallet created at {}", path);
                println!("Address: {}", wallet.address_string());
                println!("Mnemonic: {}", wallet.mnemonic.as_deref().unwrap_or("<none>"));
                return Ok(());
            }
    ...
    if cli.daemon || cli.node_type.is_some() || cli.p2p_port.is_some() || cli.rpc_port.is_some() || cli.bootnodes.is_some() {
        let handles = aether_unified::node::run_node(cfg).await?;
        tokio::signal::ctrl_c().await?;
        tracing::info!("Shutting down...");
        if let Ok(dag) = handles.dag.try_read() { let _ = save_dag_to_json(&dag, &handles.dag_store_path).await; };
        if let Ok(storage) = handles.storage.try_read() { let _ = storage.flush(); };
    } else {
        run_gui(cfg)?;
    }
    Ok(())
